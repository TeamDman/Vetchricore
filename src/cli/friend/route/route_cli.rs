...
impl FriendRouteArgs {
    ...
    pub async fn invoke(self, context: &InvokeContext) -> Result<()> {
        match self.command {
            FriendRouteCommand::Add(args) => args.invoke(context).await?,
            FriendRouteCommand::List(args) => args.invoke(context).await?,
            FriendRouteCommand::Remove(args) => args.invoke(context).await?,
        }
        Ok(())
    }
}
