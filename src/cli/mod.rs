...
use crate::cli::veilid::VeilidArgs;
...
pub enum Command {
    ...
    Home(HomeArgs),
    /// Veilid proof-of-concept commands.
    Veilid(VeilidArgs),
}
...
impl Command {
    ...
    pub async fn invoke(self) -> eyre::Result<()> {
        match self {
            ...
            Command::Veilid(args) => args.invoke().await,
        }
    }
}
...
impl ToArgs for Command {
    fn to_args(&self) -> Vec<OsString> {
        ...
        match self {
            ...
            Command::Veilid(veilid_args) => {
                args.push("veilid".into());
                args.extend(veilid_args.to_args());
            }
        }
        args
    }
